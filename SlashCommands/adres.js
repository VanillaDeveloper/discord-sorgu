...
module.exports = {
    ...
    if(!member.roles.cache.has(roller.vip) && !member.roles.cache.has(roller.premium)) return await int.followUp({embeds:[new discord.EmbedBuilder().setDescription(`Bu komutu kullanmak için premium veya vip olmalisin.`)]})

    if (veri.success === true) {

        log(`Sorgulanan TC: ${TCKN}`,`Adres Sorgu 2022 Yapildi`)
    
       await int.followUp({ embeds: [new discord.EmbedBuilder().setTitle(`${veri2[0]?.ADI} ${veri2[0]?.SOYADI}`).setDescription(`ACIK ADRES: \`${veri.Ikametgah}\``)]})
    } else {
